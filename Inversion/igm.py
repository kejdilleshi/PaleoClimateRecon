...
class Igm:
    ...
    def initialize_iceflow(self):
        ...
        Ny = self.max_thk.shape[0]
        Nx = self.max_thk.shape[1]
...
#           print('at the surface? : ',all(self.zpos == topg+othk))
    ...
    def optimize2(self):
        ...
        assert ("ela" in self.config.opti_cost) == (
            "ela" in self.config.opti_control
        )
            ...
            with tf.GradientTape() as t:
                ...
                X = tf.concat(
                    [
                        tf.expand_dims(
                            tf.expand_dims(
                                tf.pad(topg, self.PAD, "CONSTANT"), axis=0
                            ),
                            axis=-1,
                        ),
                        tf.expand_dims(
