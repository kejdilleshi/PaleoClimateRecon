"""_summary_
  Code to create an emulator. 
  Takes in as arguments topography, ELA, time,'A','beta','c'.
Returns:
    Glacier footprints
"""
...
args.epochs=10
args.clipnorm=1.0
args.model_dir='results/StandartParameters/best_model.h5'
args.data_dir='fordle'
args.dataset='OneTopo'
args.test_topo='ALPS.nc'
args.results_dir='results/test_emulator'
args.nb_blocks=4
...
def run (run_name,hparams):
    with tf.summary.create_file_writer('logs/hparam_tuning/' + run_name).as_default():
        ...
        model = model_builder.build_model(model_type=hparams[HP_Architecture], nb_inputs=6, nb_outputs=1,hparams=hparams,model_path=args.model_dir, unfrozen_layers=16)
        # For transfer-learning
        # model.load_weights(args.results_dir + '/best_model.h5')
       ...
       ## Load and preprocess data
        ...
        # Train model 
        history = model_builder.train(model, train_x, train_y, test_x, test_y,hparams,Data_augment=True)        
        ## Evaluate the trained model
        evaluator = ModelEvaluator(args,data_loader.set_scaling())
        evaluator.evaluate(model, test_x, test_y,nb_test=20)
        evaluator.plot_lc(history)
        evaluator.plot_lc_normalized(history)
