...
class DataLoader:
    ...
    def set_scaling(self):
        ...
        self.naturalbounds = {}
        #
        self.naturalbounds["gfp"]         = 3180
        self.naturalbounds["topg"]   	  = 1960
        self.naturalbounds["ela"]   	  = 340
        self.naturalbounds["time"]   	  = 6
    ...
    def load_nc_files(self):
        """
        Find and load all files with '.nc' extension in the given folder and its subfolders
        """
        nc_files = glob.glob(os.path.join(self.folder, '**/*.nc'), recursive=True)
        data = []
        idx=0
    ...
    def split_data_4(self,field_in, field_out, file_path=None, time=None):
        ...
        # Split the data into train and test sets with the specified topography
        test_idx = np.where(data.variables['time'][:] == time)[0]
        train_idx = np.where(data.variables['time'][:] != time)[0]
